...
export default function Events() {
  ...
  const fetchEvents = async (showLoader = true) => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/events?user_id=${user.id}`, {
        headers: {
  ...
  const handleRegisterEvent = async (event: Event) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/events/${event.id}/register`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const cancelBookingNow = async (event: Event) => {
    ...
    try {
      ...
      setEvents((prev) =>
        prev.map((item) =>
          Number(item.id) === Number(event.id)
            ? { ...item, registered: false, registrations: updatedRegistrations }
            : item
        )
      );
      ...
      Alert.alert("Booking cancelled", "This event has been removed from your bookings.");
      await fetchEvents(false);
    } catch (error: any) {
  ...
  const handleBookButtonPress = async (event: Event) => {
    ...
    if (isBooked) {
      Alert.alert("Booking Options", "What would you like to do?", [
        { text: "View Booking", onPress: () => goToEventBookedPage(event) },
        {
          text: "Cancel Booking",
          style: "destructive",
  ...
  const getCategoryIcon = (category: string, title: string) => {
    ...
    if (text.includes("environment") || text.includes("beach") || text.includes("park"))
      return "leaf-outline";
    if (text.includes("food") || text.includes("soup") || text.includes("kitchen"))
      return "restaurant-outline";
    if (text.includes("blood") || text.includes("health"))
      return "heart-outline";
    if (text.includes("youth") || text.includes("mentor") || text.includes("education"))
    ...
    return (
      ...
      >
        <View style={styles.cardHeader}>
          ...
          <View style={[styles.pointsBadge, { backgroundColor: theme.colors.primary + "22" }]}>
            <Ionicons name="star" size={13} color={theme.colors.primaryLight} />
            <Text style={[styles.pointsReward, { color: theme.colors.primaryLight }]}>
        ...
        <View style={styles.orgRow}>
          ...
          <Text style={[styles.orgText, { color: theme.colors.textSecondary }]} numberOfLines={1}>
        ...
        <View style={[styles.detailsBox, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}>
          <View style={styles.detailRow}>
            <View style={[styles.detailIconBox, { backgroundColor: categoryColor + "22" }]}>
              <Ionicons name="calendar-outline" size={15} color={categoryColor} />
            </View>
          ...
          <View style={styles.detailRow}>
            <View style={[styles.detailIconBox, { backgroundColor: categoryColor + "22" }]}>
              <Ionicons name="location-outline" size={15} color={categoryColor} />
            </View>
        ...
        <View style={styles.capacitySection}>
          <View style={styles.capacityTopRow}>
            <Text style={[styles.capacityText, { color: theme.colors.text }]}>Capacity</Text>
            <Text style={[styles.capacityCount, { color: isFull ? "#ef4444" : theme.colors.textSecondary }]}>
          ...
          <View style={[styles.progressBg, { backgroundColor: theme.colors.border }]}>
            <View
              style={[
                styles.progressFill,
                { width: `${progress}%` as any, backgroundColor: isFull ? "#ef4444" : categoryColor },
              ]}
            />
          </View>
        </View>
        ...
        >
          {isSubmitting ? (
            <ActivityIndicator size="small" color={isBooked ? categoryColor : "#fff"} />
          ) : (
            <>
              <Ionicons
                name={
                ...
                size={18}
                color={isBooked ? categoryColor : isFull ? theme.colors.textTertiary : "#fff"}
              />
              <Text
                style={[
    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: theme.colors.background }]}>
      ...
      {loading ? (
        ...
        <FlatList
          data={events}
          keyExtractor={(item) =>
            `${item.id}-${item.registered ? "booked" : "open"}-${item.registrations ?? 0}`
          }
...
const styles = StyleSheet.create({
  ...
  pointsReward: { fontSize: 12, fontWeight: "900" },
  eventTitle: { fontSize: 19, fontWeight: "900", marginBottom: 8, lineHeight: 24 },
  orgRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 14 },
  orgText: { fontSize: 12, fontWeight: "600", flex: 1 },
  detailsBox: { borderWidth: 1, borderRadius: 18, padding: 14, marginBottom: 14, gap: 10 },
  detailRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  detailIconBox: { width: 30, height: 30, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  detailText: { fontSize: 13, fontWeight: "600", flex: 1 },
  eventDescription: { fontSize: 14, lineHeight: 20, marginBottom: 16 },
  capacitySection: { marginBottom: 16 },
  capacityTopRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8 },
  capacityText: { fontSize: 12, fontWeight: "800" },
  capacityCount: { fontSize: 12, fontWeight: "700" },
  ...
  loadingContainer: { alignItems: "center", justifyContent: "center", marginTop: 70 },
  loadingText: { marginTop: 12, fontSize: 14, fontWeight: "600" },
  emptyContainer: { alignItems: "center", marginTop: 80, paddingHorizontal: 40 },
  emptyIconBox: { width: 104, height: 104, borderRadius: 32, alignItems: "center", justifyContent: "center", marginBottom: 18 },
