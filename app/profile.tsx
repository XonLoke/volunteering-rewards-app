...
const menuItems = [
  ...
  { id: "2", icon: "stats-chart-outline", label: "Points History", sub: "See your earned points", route: "/points-history" },
  { id: "3", icon: "scan-outline", label: "Scan History", sub: "All past QR scans", route: "/scan-history" },
  { id: "4", icon: "settings-outline", label: "Settings", sub: "App preferences", route: "/settings" },
];

const supportItems = [
  { id: "5", icon: "help-circle-outline", label: "Help & FAQ", sub: "Get answers fast", route: "/help" },
  { id: "6", icon: "call-outline", label: "Contact Us", sub: "We're here to help", route: "/contact" },
];
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      try {
        ...
        // Fetch coupons count
        const couponsRes = await fetch(`${BASE_URL}/my-coupons?user_id=${parsedUser.id}`);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.heroBanner, { backgroundColor: theme.colors.surface }]}>
          ...
          <View style={styles.avatarArea}>
            ...
            <Text style={[styles.nameText, { color: theme.colors.text }]}>{user?.name || "Guest"}</Text>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statsStack}>
            <View style={[styles.statMini, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              <Text style={[styles.statMiniNum, { color: theme.colors.text }]}>{scansCount}</Text>
              <Text style={[styles.statMiniLabel, { color: theme.colors.textSecondary }]}>Scans</Text>
            </View>
            <View style={[styles.statMini, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              <Text style={[styles.statMiniNum, { color: theme.colors.text }]}>{couponsCount}</Text>
          ...
          <View style={[styles.menuCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            {menuItems.map((item, index) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <View style={[styles.menuIconBg, { backgroundColor: accentSoft }]}>
          ...
          <View style={[styles.menuCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            {supportItems.map((item, index) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <View style={[styles.menuIconBg, { backgroundColor: gold + "18" }]}>
...
const styles = StyleSheet.create({
  ...
  blob1: { position: "absolute", width: 180, height: 180, borderRadius: 90, top: -40, right: -40 },
  blob2: { position: "absolute", width: 120, height: 120, borderRadius: 60, bottom: 10, left: -30 },
  topBar: {
  ...
  avatarRing: { width: 110, height: 110, borderRadius: 55, borderWidth: 3, padding: 4, marginBottom: 16 },
  avatarRingInner: { flex: 1, borderRadius: 50, borderWidth: 2, overflow: "hidden" },
  avatarImg: { width: "100%", height: "100%", borderRadius: 50 },
  avatarFallback: { flex: 1, alignItems: "center", justifyContent: "center", borderRadius: 50 },
  camBadge: {
  ...
  editPhotoPill: { paddingVertical: 7, paddingHorizontal: 20, borderRadius: 20, borderWidth: 1 },
  editPhotoLabel: { fontSize: 13, fontWeight: "700" },
  statsRow: { flexDirection: "row", paddingHorizontal: 20, paddingTop: 20, paddingBottom: 8, gap: 12 },
  statHero: { flex: 1.2, borderRadius: 22, padding: 20, justifyContent: "center", overflow: "hidden", position: "relative" },
  ...
  statHeroDot: { position: "absolute", width: 80, height: 80, borderRadius: 40, backgroundColor: "rgba(255,255,255,0.15)", bottom: -20, right: -20 },
  statsStack: { flex: 1, gap: 12 },
  statMini: { flex: 1, borderRadius: 18, borderWidth: 1, alignItems: "center", justifyContent: "center", paddingVertical: 12 },
  ...
  menuIconBg: { width: 42, height: 42, borderRadius: 13, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  menuTextCol: { flex: 1 },
  menuLabel: { fontSize: 15, fontWeight: "700", marginBottom: 2 },
  menuSub: { fontSize: 11, fontWeight: "500" },
  arrowBubble: { width: 28, height: 28, borderRadius: 9, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  logoutBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginHorizontal: 20, marginTop: 28, borderRadius: 16, borderWidth: 1.5, borderColor: "#ef4444", paddingVertical: 15, backgroundColor: "#ef444410" },
  logoutText: { color: "#ef4444", fontSize: 15, fontWeight: "800", letterSpacing: 0.3 },
  versionTag: { textAlign: "center", marginTop: 20, fontSize: 11, fontWeight: "500" },
});