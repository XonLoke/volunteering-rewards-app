import {
  Text, View, TouchableOpacity, SafeAreaView, StyleSheet,
...
export default function Settings() {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (saved) {
          ...
          setLocationAccess(s.locationAccess ?? true);
        }
      } catch (err) {
  ...
  const saveSettings = async (key: string, value: boolean) => {
    try {
      ...
      await AsyncStorage.setItem("settings", JSON.stringify({ ...current, [key]: value }));
    } catch (err) {
  ...
  const toggles = [
    ...
    },
    {
      ...
      sub: "Receive updates and news via email",
      value: emailNotifs,
      key: "emailNotifs",
    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: theme.colors.background }]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            {toggles.map((item, index) => (
              ...
              >
                ...
                <Switch
                  value={item.value}
                  onValueChange={(val) => handleToggle(item.key, item.setter, val)}
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <TouchableOpacity
              style={[styles.expandRow, showPasswordSection && { borderBottomWidth: 1, borderBottomColor: theme.colors.border }]}
