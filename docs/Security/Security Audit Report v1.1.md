# Security Audit Report v1.1

**Date:** 20 July 2026
**Project:** Volunteering Rewards App (C3000C)
**Performed by:** Xon (Technical Lead)
**Branch:** `main` (commit `8764c4f`)
...
All defaults are configurable via environment variables (`AUTH_STRICT_MAX`,
`AUTH_REGISTER_MAX`, `AUTH_STRICT_WINDOW_MS`, `AUTH_REGISTER_WINDOW_MS`).
Rate limiting can be disabled entirely for local development by setting
`DISABLE_RATE_LIMIT=true` — this is intended for `NODE_ENV=development` only.
...
| 10 | `backend/src/middleware/rateLimiter.middleware.js` | Rate limits made env-configurable; added `DISABLE_RATE_LIMIT` dev switch |
...
*— End of Security Audit Report v1.1 —*
