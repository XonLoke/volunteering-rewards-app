# Native APK Build — Troubleshooting & Solution Report (V3)

**Project:** Volunteer Rewards App  
**Date:** 29 June 2026  
**Author:** Claude Code (assisting developer)  
**Version:** 3 (CI build also successful on GitHub Actions)  
**Status:** ✅ BUILD SUCCESSFUL — Both Local (83 MB) and CI (GitHub Actions) ✅

> **Key insight:** The real solution was upgrading Expo SDK 52 → 54 in `frontend/mobile_app/`.  
> The CI workflow failures were symptoms of `main` branch still running SDK 52 (`expo-modules-core@2.2.3`), which lacks the required Gradle plugin. Once the SDK upgrade was merged to `main`, the CI build succeeded.
...
### 5.3 CI Build on GitHub Actions (Subsequently Successful ✅)

After merging all fixes (SDK upgrade, import paths, splash screen placeholder, workflow caching fix) to `main`, the CI workflow also succeeded.

| Check | Result |
|-------|:------:|
| CI workflow | ✅ `build-apk.yml` on `main` |
| Build type | Debug |
| Platform | `ubuntu-latest` |
| Duration | ~5 minutes first build |
| Artifact | Available in Actions run → Artifacts |

**Root cause of CI failures:** The `main` branch still had Expo SDK 52 (`expo-modules-core@2.2.3`), which lacks the `expo-module-gradle-plugin`. The CI workflow's verification step correctly rejected the SDK 52 setup. All CI workflow adjustments (SDK 36, caching fix, splash placeholder) were secondary — the primary fix was merging the SDK 52→54 upgrade to `main`.

**What was fixed in the CI workflow:**
1. Removed `cache: gradle` from `setup-java@v4` — the `android/` directory doesn't exist until `expo prebuild` runs
2. Added splash screen placeholder PNG creation — `expo prebuild` references this drawable but doesn't create it
3. Added `chmod +x gradlew` — files generated by `expo prebuild` may not have execute permissions
